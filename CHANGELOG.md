...
- sa2content: playable Unknown (Slow Joe body + runtime Shotgun clone with every damage effect but mind control) behind `SA2_ENABLE_UNKNOWN`, OFF by default. The setup itself runs, but the game then crashes opening the map (SIGSEGV in `MapLevelIcon.TrySetupRewardIcon`); see the sa2content README.
